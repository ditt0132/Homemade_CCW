...
object QuickSlotManager {
    fun inv(player: Player): Inventory {
        val inv = Bukkit.createInventory(QuickSlotInvHolder(), 45, "§8Quick Slot Manager")
        ...
        inv.setItem(44, ItemManager.createNamedItem(Material.REDSTONE_TORCH, 1, "§c도움말", listOf("§7/닭갈비 퀵슬롯 (숫자) 명령어로 퀵슬롯을 지정할 수 있습니다!", "§7쉬프트 + F(양손 들기 키)로 이 메뉴를 열 수 있습니다.")))
        ...
        for (slot in 0..17) {
            ...
            if (itemUsing.type != Material.BARRIER) addLoreLine(itemUsing, "§aClick to Quick Equip")
            addLoreLine(itemUsing, "§e쉬프트 + 좌클릭으로 현재 들고 있는 아이템으로 퀵슬롯을 지정할 수 있습니다.")
            if (slot <= 8) {
                val invSlot = slot + 9
    ...
    fun setQuickSlot(player: Player, slot: Int) {
        val data = initData(player.uniqueId)
        if (slot !in 0..17) {
            player.sendMessage("§c슬롯은 0부터 17사이의 숫자여야 합니다.")
            return
        }
        if (player.inventory.itemInMainHand.type == Material.AIR) {
            player.sendMessage("§c퀵슬롯 ${slot}번이 삭제되었습니다.")
        ...
        if (item.itemMeta.displayName().toString().contains("/-&-/")) {
            player.sendMessage("§c이름에 /-&-/기호가 들어가면 안됩니다.")
            return
        }
        if (item.itemMeta.displayName().toString().contains("/-:-/")) {
            player.sendMessage("§c이름에 /-:-/기호가 들어가면 안됩니다.")
        ...
        player.sendMessage("§a해당 아이템이 퀵슬롯 ${slot}번으로 설정되었습니다..")
        player.inventory.close()
    }
